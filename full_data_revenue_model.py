...
def train_full_data_revenue_model():
    """
    Train a revenue prediction model using 100% of the modified dataset.
    Modified dataset no longer contains 'Total Cost', 'Profit', and 'Profit Margin (%)'.
    """
    print("Training revenue model with FULL dataset (100% of data)...")
    ...
    # Add seasonal product statistics
    for df_name, df in [('train', train_df), ('test', test_df)]:
        if 'Quarter' in df.columns and '_ProductID' in df.columns:
            if df_name == 'train':
                # Only compute this on training data to avoid data leakage
                seasonal_stats = train_df.groupby(['_ProductID', 'Quarter'])['Total Revenue'].agg(['mean', 'count']).reset_index()
                seasonal_stats.columns = ['_ProductID', 'Quarter', 'Season_Product_Avg', 'Season_Product_Count']
            
            # Merge with both datasets
            df = pd.merge(df, seasonal_stats, on=['_ProductID', 'Quarter'], how='left')
            
            # Fill missing values
            df['Season_Product_Avg'] = df['Season_Product_Avg'].fillna(df['Avg_Product_Revenue'])
            df['Season_Product_Count'] = df['Season_Product_Count'].fillna(1)
            
            # Create seasonality impact feature
            df['Season_Impact'] = df['Season_Product_Avg'] / (df['Avg_Product_Revenue'] + 1)
    ...
    xgb_search = RandomizedSearchCV(
        ...
        n_iter=12,  # Reduced number of iterations for faster completion
        cv=3,       # Reduced CV folds for faster completion
        random_state=42,
        n_jobs=-1,
        verbose=1,
        scoring='r2'
    ...
    joblib.dump(model_data, 'best_revenue_model_full_data.pkl')
    joblib.dump(encoders, 'revenue_encoders_full_data.pkl')
    
    print("\nModel and encoders saved to:")
    print("- best_revenue_model_full_data.pkl")
    print("- revenue_encoders_full_data.pkl")
    ...
    if hasattr(final_model, 'feature_importances_'):
        ...
        plt.tight_layout()
        plt.savefig('feature_importance_full_data.png')
    ...
    plt.tight_layout()
    plt.savefig('actual_vs_predicted_full_data.png')
    
    return final_model, encoders, features

if __name__ == "__main__":
    train_full_data_revenue_model() 