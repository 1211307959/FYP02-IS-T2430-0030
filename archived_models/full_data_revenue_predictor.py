...
def load_revenue_model():
    """
    Load the full data revenue prediction model and its encoders.
    """
    try:
        model_data = joblib.load('best_revenue_model_full_data.pkl')
        model = model_data['model']
        features = model_data['features']
        log_transform = model_data.get('log_transform', True)
        encoders = joblib.load('revenue_encoders_full_data.pkl')
...
def preprocess_input(input_data, encoders, features):
    """
    Preprocess input data for prediction with the full data model.
    """
    ...
    # Calculate derived features that were in the original dataset
    if 'Unit Price' in processed.columns and 'Unit Cost' in processed.columns:
        ...
        processed['Profit Margin (%)'] = (processed['Margin_USD'] / processed['Unit Price']) * 100
        
        # For Total Cost, we need to estimate it if Order Quantity is available, otherwise use a default
        if 'Order Quantity' in processed.columns:
            processed['Total Cost'] = processed['Unit Cost'] * processed['Order Quantity']
        else:
            # Default to Unit Cost (will be adjusted with feature engineering)
            processed['Total Cost'] = processed['Unit Cost']
    ...
    # These defaults should ideally be calculated from the training data or provided as a configuration
    ...
    # Add seasonal features with reasonable defaults
    if 'Season_Product_Avg' in features and 'Season_Product_Avg' not in processed.columns:
        processed['Season_Product_Avg'] = processed.get('Avg_Product_Revenue', 10000)
    
    if 'Season_Product_Count' in features and 'Season_Product_Count' not in processed.columns:
        processed['Season_Product_Count'] = 10  # Default count
    
    if 'Season_Impact' in features and 'Season_Impact' not in processed.columns:
        processed['Season_Impact'] = 1.0  # Neutral impact
...
def predict_revenue(input_data):
    """
    Predict revenue using the full data model.
    """
...
def simulate_price_variations(input_data):
    ...
    return {
        ...
        "unit_cost": unit_cost,
        "model_type": "full data revenue prediction (no quantity input)",
